...
class Projects:
    ...
    def save(self):
        ...
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO projects (user_id, project, approval, popup1, popup2, popup3, popup4, payment_status)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
        ''', (self.user_id, self.project, self.approval, self.popup1, self.popup2, self.popup3, self.popup4, self.payment_status))
        conn.commit()
        cursor.close()
        conn.close()
    ...
    def get_all_projects(user_id):
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True)
        cursor.execute('SELECT users.full_name, users.email, users.whatsapp_number, users.WelcomeMessage, users.business_name, users.offers, users.popup_text, users.whatsapp_message, projects.* FROM projects LEFT JOIN users ON projects.user_id=users.id WHERE projects.user_id = %s', (user_id,))
